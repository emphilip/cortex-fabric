...
The ingestion service SHALL expose connectors through both a CLI binary AND an HTTP surface. The container SHALL by default run a FastAPI HTTP server bound to port `8100` inside the compose network. The `hive-mind-ingest` CLI binary MUST remain callable via `docker compose exec ingestion hive-mind-ingest …`. Both surfaces MUST share the same underlying connector implementations — no duplicate code paths.

Background: the thin-MVP version specified ingestion as a CLI binary only; this change keeps the CLI working unchanged AND adds the HTTP surface so the admin UI can trigger runs and read status without using `docker compose exec`.
...
Unchanged from `bootstrap-thin-mvp`: the git connector SHALL clone, walk, chunk, embed, and upsert. This change additionally requires that a git ingest triggered via `POST /run/git` SHALL run in a background task so the HTTP response is immediate and status is observable via `GET /runs/recent`.
...
Run history MAY be in-memory only in this change. Durability across container restarts is explicitly deferred to a follow-up change.
