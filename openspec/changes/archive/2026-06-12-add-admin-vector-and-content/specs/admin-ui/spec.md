...
Unchanged from `bootstrap-thin-mvp`: the admin UI SHALL NOT expose any graph relationship management surface in v1 either. The `add-knowledge-graph` change adds it.

#### Scenario: Graph management still absent

- **WHEN** an operator navigates the admin UI after this change ships
...
Every new shared component introduced by this change (`EntityRow`, `EntityDetail`, `VectorHit`, `ConnectorCard`, `IngestionRunRow`) MUST have a neighbour `*.stories.tsx` file with at least three stories covering the canonical, empty, and error states.
