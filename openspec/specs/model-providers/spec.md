...
The `hive-mind.yaml` config SHALL gain a `providers` section with sub-blocks `embeddings` and `chat`. Each sub-block carries `provider`, `model`, `base_url`, and optional `api_key`. The existing `ollama` block stays for backwards compatibility (its values populate `providers.embeddings` if `providers.embeddings` is absent). Environment overrides MUST work via `HIVE_MIND__PROVIDERS__CHAT__MODEL` etc.
...
- **WHEN** a deployment's `hive-mind.yaml` still has only the old `ollama:` block (no `providers:` section)
...
- **WHEN** `HIVE_MIND__PROVIDERS__CHAT__MODEL` is set to `kimi-k2`
