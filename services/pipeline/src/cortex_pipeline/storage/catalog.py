...
class CatalogStore:
    ...
    async def list_entities(
        ...
        *,
        ...
        """
        count_sql = f"SELECT count(*) FROM cortex.entity WHERE {where}"
    ...
    async def get_evidence_chunks(
        ...
        async with pool.acquire() as conn:
            rows = await conn.fetch(
                ...
                FROM cortex.relationship_evidence evidence
                JOIN cortex.relationship_edge edge
                  ON edge.edge_id = evidence.edge_id
                JOIN cortex.entity entity
