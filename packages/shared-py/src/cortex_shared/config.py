"""Config loader. YAML file + CORTEX__SECTION__KEY env overrides."""
...
class PostgresCfg(BaseModel):
    url: str = "postgresql://cortex:cortex@postgres:5432/cortex"
...
_ENV_PREFIX = "CORTEX__"


def _apply_env_overrides(data: dict[str, Any]) -> dict[str, Any]:
    """Apply CORTEX__SECTION__KEY env vars onto a nested dict."""
...
def load_config(path: str | Path | None = None) -> CortexConfig:
    ...
    raw: dict[str, Any] = {}
    if path is None:
        path = os.environ.get("CORTEX_CONFIG", "cortex.yaml")
    p = Path(path)
    if p.is_file():
